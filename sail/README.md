...
- **Spec feed (`sail spec`, #60/#75):** the front door fetches the issue with
  `gh issue view <n> --json title,body,comments,author` and pipes it through `python3 -m sail spec`,
  ...
  partial spec. Comments are a prompt-injection surface on shared repos (#75): they are emitted
  inside an explicit untrusted-data fence (with in-body fence/header sentinels neutralized), and
  `SAIL_COMMENT_TRUST=all|author|none` controls exposure — `all` (default) keeps every comment
  fenced, `author` keeps only the issue author's comments, `none` drops comments entirely; an
  unrecognized value fails closed (exit 1).
...
### Convergence oracle — layered decision order (#77, #103, #108, #130)

`python3 -m sail converge --rc R --round N --run-dir DIR --target .` is the deterministic loop
decision the autonomous driver consults. It prints exactly one of
`proceed | revise | park | proceed-hardening | proceed-dissent` and surfaces per-run cost
(wall-time elapsed, read from `run-state.json` `started_at`) to stderr. On a non-green round the
...
- **Backend choice (#83):** `claude -p` (Sonnet 5) keeps the implementer in a different family from a codex review lens, preserving cross-family review; `codex exec …` is cheapest but collides with a codex `SAIL_REVIEW_CMD2`. If `SAIL_BUILD_CMD` is set, `SAIL_REVIEW_CMD` is the single-lens `claude` reviewer, and `build.json` comes back `inline` with `reason=backend-not-runnable`, raise an **ALERT** because builder=reviewer=claude and cross-family review was lost. `sail build` emits an advisory `same_family_warning` (best-effort, wrapper-aware, non-blocking) when the selected build backend and an active review lens share a family. Inline stays the default/fallback only when `SAIL_BUILD_CMD` is unset.
