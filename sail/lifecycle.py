...
def land_comment(issue, review_data, resolutions, review_state="ok"):
    ...
    if review_state == "missing":
        ...
        # booleans — the durable, freshness-keyed artifact — so no separate note store is needed.
        from sail.review import degraded_lenses, format_degraded_note
        degraded = degraded_lenses(review_data) if isinstance(review_data, dict) else []
        if degraded:
            out += [format_degraded_note(degraded).rstrip(), ""]
