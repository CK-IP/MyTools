...
- **Inherited `SAIL_*` env knobs:** a real shell exports the sail backend commands (settings.json: `SAIL_PLAN_GROUNDED_CMD`, `SAIL_PLAN_CMD2`, `SAIL_REVIEW_CMD2`, `SAIL_REDTEAM_CMD`, …), so any subtest that depends on a backend being *absent* will inherit live `codex` and invoke it for real — flaking on nondeterministic output (the failure even hops between subtests across runs). Mock backends are not enough if the env still points at a real one. Clear them once at the top, after `cd "$REPO_ROOT"`, with `unset "${!SAIL_@}"`; subtests that need a backend still set it via their command prefix (which overrides the unset for that one invocation). `unset "${!SAIL_@}"` with zero matches is a no-op returning 0 under `set -e`, so it is CI-safe. (Sibling of the "prefix env-vars, never trail" rule above — both are about the test controlling its own env.)

The general rule: a sail test controls exactly what it sees and depends on no live `git`/working-tree/branch state **and no inherited `SAIL_*` env configuration**. (Same family as the worktree-scanning rule above.)

*Source: #68 — `--target $REPO_ROOT` caused a T9 false-fail; #78 — #56-T4/#64-T6 asserted `git diff main -- sail/*.py` and false-failed on every `.py`-touching sibling branch (rewritten to inspect the checker registry); #97 — test_sail_93 T1c/T4 inherited `SAIL_PLAN_GROUNDED_CMD`/`SAIL_PLAN_CMD2` and invoked live codex, flaking nondeterministically (fixed with `unset "${!SAIL_@}"`).*
