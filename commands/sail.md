...
**`SAIL_BUILD_CMD` pluggable build backend (#95).** Default build is **inline** — the orchestrating session writes the failing test then the minimum code. To delegate the expensive code-writing (and the per-round convergence fixes in Stage 3) to a configurable fast backend, set `SAIL_BUILD_CMD` and run:
...
Branch on `build.json`'s `status`: **`delegated`** → the backend wrote the change; proceed to review. **`inline`** → no runnable backend; build inline TDD-guided as today (clean degrade, logged, not an error). **`error`** → **fail closed** (a missing `.sail/last-test-failed` marker, a backend error, or — in fix-mode — a missing/incomplete `review.json`); halt and surface, do not proceed. TDD is **enforced** on the delegated path: `sail build` fails closed unless a failing-test marker (`.sail/last-test-failed`) is on record — the failing test is authored first (the cheap part stays inline/leadsman-owned), only the code-writing is delegated, and the backend is told never to weaken or delete the failing test. The inline path stays the default/fallback until measured.

**Backend choice — codex vs Sonnet 4.6 (#83).** Both work; document both. **codex (`gpt-5.4-mini`)** is cheapest, but if you also run `--dual-lens` with `SAIL_REVIEW_CMD2="codex …"`, codex becomes both the implementer **and** review lens2 → same-family self-review (the #83 rubber-stamp risk). **Sonnet 4.6 (`claude -p`)** keeps the implementer in a **different family** from the codex review lens, preserving genuine cross-family review of the implementation. **Recommended: Sonnet 4.6 for `SAIL_BUILD_CMD`** whenever a codex review lens is active; keep lens1 = `claude`. `sail build` emits an advisory `same_family_warning` in `build.json` when `SAIL_BUILD_CMD` and `SAIL_REVIEW_CMD2` resolve to the same backend family (best-effort, wrapper-aware, non-blocking).
