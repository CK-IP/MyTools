...
**Mutation verification (`mutation.verify` / `mutation-verify`, #131).** After the green TDD build, run the executable mutation-verify check for diffs that introduce at least one new/changed test file and at least one non-test source change. It is a deterministic no-op unless that shape holds; `--title` still feeds the deterministic `is_bug_fix_title` provenance check and `--bug-fix` remains an explicit operator override, but neither one gates the trigger anymore. Pass the raw issue title and let the engine decide — `python3 -m sail mutation-verify --target . --diff <base-ref> --run-dir "$SESSION_DIR" --title "$TITLE"`: the bug-fix decision is the **deterministic, unit-tested `is_bug_fix_title` predicate** in `sail/mutation_verify.py` (a conventional-commit `fix`/`bugfix`/`hotfix` title), **not** an orchestrator judgment call — so always pass `--title` rather than deciding bug-fix-ness in this prompt (CLAUDE.md infra-placement). The runtime budget is controlled by `SAIL_MUTVERIFY_BUDGET_SECONDS` (also accepted: `SAIL_MUTATION_VERIFY_BUDGET_SECS`); when it is exceeded, remaining test re-runs are skipped with an informational stderr note containing the budget. When it fires, the check reverts only the source hunks, reruns the new/changed tests as a collective verdict, and restores the tree in `try/finally`; if the restore itself fails, the run exits non-zero and reports the partially reverted tree state loudly instead of continuing silently. A clean pass under the revert emits `category: test-adequacy` findings tagged `lens: mutation-verify` into the existing review pipeline (freshness-gated by `diff_hash`). This complements, and does not replace, the #70 inline mutation-survival probe in Stage 3.
...
**Risk-scaled review depth — a SECOND, differently-focused perspective on high-stakes diffs, no flag (#148).** Heterogeneous review perspectives pay on high-stakes paths (in a 146-PR study, 93.4% of bugs were caught by exactly ONE of four review tools, none by all four). So on an `is_high_stakes(diff)` diff `/sail` **automatically widens review depth to two perspectives — no flag required** (`review_perspectives()` in `sail/review.py` is the deterministic, tested depth selector). Where the repo-exploring **red-team** already fires (see below), or an explicit `--dual-lens` lens2 runs, that IS the second perspective and nothing extra is added (the selector is second-perspective-aware — never a redundant extra read of the same diff). Where it does not (a non-spine high-stakes diff with no `SAIL_REDTEAM_CMD`), `/sail` falls back to a same-family **focus** pass: it re-uses the **primary review backend** (`SAIL_REVIEW_CMD`, claude) with a **distinct** security + spec/interface-compliance prompt — *not* a duplicate correctness pass — and unions its findings tagged **`lens: "focus"`** in `review.json`, blocking on CRITICAL/HIGH via the same path as lens2/red-team.

- **No codex consumption by default — the hard constraint (#83, 2026-06-27 policy).** The 2026-06-27 codex model policy **deliberately dropped `--dual-lens` by default** to conserve codex (cross-family review is achieved by the codex-build / claude-review flip). The focus perspective therefore **must not** re-introduce a codex lens2: it is bound to the **same-family** primary review backend (claude), never `SAIL_REVIEW_CMD2`. It is a *differently-focused* pass, not a *different-family* one — so it adds **zero** codex cost. **Do not** wire the focus fallback to `SAIL_REVIEW_CMD2`.
- **Low-stakes pays nothing.** The selector returns single-lens for ordinary / advisory / empty diffs — unchanged, no extra pass.
- **Degrades cleanly, consistent with #116.** The focus pass **shares lens1's backend**, so there is no independent single-lens-on-high-stakes hole: if the primary backend is absent the whole review fails closed (existing behavior); a focus backend *error* fails closed (never-mask). Because focus is same-family, it is intentionally **not** tracked in the #116 cross-family degradation set (`_CROSS_FAMILY_LENSES`) — it never manufactures a spurious cross-family ALERT. The **depth choice is folded into the review reuse/freshness decision** as a companion to the `diff_hash`/`plan_hash` keys: `depth_reuse_ok()` in `sail/review.py` (the owner of the depth semantics) decides whether a cached review satisfies the current diff's required depth, and the runner consults it at resume — a resumed high-stakes run whose cache lacks the focus lens is re-reviewed rather than reused single-lens, so the two-perspective floor survives resume.
...
- **Shared negative prompting + churn signal.** The review, red-team, and tidiness prompts all append the same `DO_NOT_FLAG` section: do not flag pre-existing style/naming nits outside the diff, speculative hardening without a concrete exploit path, patterns outside the diff, doc-tone/phrasing issues, or items already covered by deterministic gates (lint/type/security scanners). Each review round also records `- advisory-findings [round=N]: K` in the decision log, where `K` is the current count of non-blocking review findings; re-running the same round overwrites that round's row instead of appending another one.
