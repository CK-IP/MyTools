#!/bin/bash
# Sloop Stop hook — enforces /sloop Stage 2→3 auto-transition.
...
# 4. sentinel absent → exit 0 (not a sloop run)
...
[ -f "$HOME/.ship/sloop-active-$PPID" ] || exit 0
...
msg="/sloop Stage 2 has returned. You must dispatch @red-team (Stage 3) before ending your turn — or present the anomalous /implement return AskUserQuestion. See sloop.md Stage 2 auto-transition rule."
printf '%s\n' "$msg" >&2
exit 2
