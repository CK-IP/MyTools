...
> **Cap classification is single-sourced in `sail/` — no new symlink (#163).** `config/surf-resume.sh` no longer parses cap notices in bash; it delegates classification + dual-horizon reset parsing to `python3 -m sail cap-recovery`, the same tested module the `/surf` supervisor uses (`sail/cap_recovery.py`). Because `sail/` is a local package invoked as `python3 -m sail` (not symlinked — see the top of this file), **no new symlink or lib mapping is required**; the watcher simply `cd`s to the repo root (which it already computes from its own path) before invoking the module, since launchd sets no working directory. Optional tuning knobs are documented in `home/settings.reference.json` (`SAIL_CAP_RECOVERY_*`: wall-clock ceiling, post-reset margin/jitter, backoff floors).
