...
surf_worker_command() {
  ...
  # terminus. #172 HARDENING: the original clause named only the initial build/review, so a heavy
  # MULTI-ROUND issue re-tripped the trap by backgrounding a LATER-round stage ("Re-run plan round 2",
  # "Round-3 mutation-verify + escalated review" — observed parking #151/#126). The clause below now
  # explicitly covers EVERY convergence round (plan re-runs, per-round mutation-verify, escalation)
  # and carries a pre-turn-end SELF-ATTESTATION check. NOTE: this is the #139 backgrounding-hardening
  # ONLY; it does NOT address the macOS screen-lock/display-sleep reaper (#157, cc#72851) that was the
  # separate proximate cause of the #151/#154/#155 idle-time reaps — that is tracked in #157.
  # The clause is a FIXED string with NO double-quotes / backticks / `$` so it stays
  ...
  local contract='=== HEADLESS-WORKER CONTRACT (overrides any default async behavior) === You are a headless claude -p worker: your process EXITS when this turn ends and you are NOT re-invoked, so any work you background is LOST. Run EVERY /sail stage SYNCHRONOUSLY in this single turn, through EVERY convergence round — not only the first: plan (and every plan re-run round), build (codex), review (codex, all rounds), per-round mutation-verify, escalated / red-team review, and commit. This EXPLICITLY covers the LATER-round stages a heavy multi-round issue re-trips on (the #172 turn-end trap) — a Re-run plan round N, a Round-N mutation-verify, an escalated review — they too MUST run inline in this same turn, NOT just the initial build/review. Do NOT use run_in_background, do NOT background a command with an ampersand, do NOT use ScheduleWakeup, do NOT spawn a local_bash background task for ANY stage or round, and do NOT background a stage and end your turn expecting to be re-invoked. Instead, BLOCK in-turn: invoke each codex build and review (on every round) as a foreground call and WAIT for it to return (a blocking foreground wait is correct and required — it is NOT the prohibited behavior), then proceed straight to the next stage in this same turn. A -p worker is NOT bound by any 10-minute tool cap, so long blocking codex calls are expected and fine. Drive all the way to the Stage-4 commit terminus before ending the turn. SELF-ATTESTATION: before ending your turn, confirm explicitly that NO stage and NO convergence round was backgrounded (no run_in_background, no ampersand, no ScheduleWakeup, no local_bash background task left pending) — only end once that check passes and /sail has committed or genuinely parked with a wip-handoff.md.'
